function [Thresholds,Stage_FPRs] = chesnayeThresholds_BACKUP(K,alphaVector,gammaVector,Resolution, validate,NumT)
...
if (length(gammaVector)<k)
    ...
    aThresholds(k)	= -2*log( alphaVector(1) );                                                 % for the first stage, we don't need to do any convolutions
    gThresholds(k)	= -2*log(1- gammaVector(1) );
