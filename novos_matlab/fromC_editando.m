...
% parameters
Gamma_k         = [0.2, 0.4,0.25];              % <vetor de gammas>
...
GThresholds(k)  = findIndex(Chi2_Norm, Gamma_k(k),1); % <identico ao superior, porem para gamma>
TruncInd_R_G    = round(GThresholds(k));
% *Resolution); 
...
NullTrunc(1:TruncInd_R_G)   = zeros(1, length(NullTrunc(1:TruncInd_R_G)));
