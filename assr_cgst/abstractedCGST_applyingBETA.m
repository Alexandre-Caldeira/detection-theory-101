...
% parameters
K               = 12;                            % analyse data 3 times
M = 40;
TotalAlpha      = 0.13;                         % pre-specified alpha-level for the full sequential test
...
Xvalues         = 0:1/Resolution:1;            % the x-axis along which the distribution of the test statistic will be defined
Null         	= betapdf(Xvalues, 1, M-1); %chi2pdf(Xvalues,2);           % null distribution for test statistic at stage k
Null            = Null/sum(Null);             	% normalise 
Chi2_Norm       = Null/sum(Null);             	% normalise 

% stage 1 threshold
k               = 1;                            % stage 1
aThresholds(k)	= 1 - Alpha_k(k).^(1./(M-1))   % for the first stage, we don't need to do any convolutions
